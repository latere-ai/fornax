...
**Add a second deploy mode: install the `fornax` binary on a host and
...
```
models/<name>.yaml   ->   fornax serve --manifest   ->   engine
        ^                          ^
        |                          |
...
`fornax` is already CGO-free, so it cross-compiles with nothing but
...
```ini
# Generated by `fornax install` from models/<name>.yaml — do not edit.
[Unit]
Description=fornax <name> (<runtime> on <count>x<gpu>)
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
ExecStart=/usr/local/bin/fornax serve --manifest /etc/fornax/<name>.yaml
Restart=on-failure
RestartSec=10
...
A test asserts every checked-in unit is byte-identical to what `fornax
...
`fornax install --manifest <path>` writes the unit, installs the
manifest to `/etc/fornax/`, and reloads systemd — but only when the unit
...
`fornax` binary, uses the `serve` verb, and names this model's manifest.
...
- **AC4** `fornax install` writes the unit and manifest, is idempotent
...
- **AC4a** The unit is generated from the manifest, and a test asserts
  every checked-in unit matches what `fornax install` renders with
  default options.
