...
same `fornax serve --manifest` entrypoint the fleet uses.**
...
| `fornax serve` | `/healthz`, `/ready`, `/metrics`, OpenAI passthrough, and the Anthropic surface at `/v1/messages` |
...
flowchart LR
  ...
  F3 --> R1["fornax serve --manifest"]
  L3 --> R2["fornax serve --manifest"]
