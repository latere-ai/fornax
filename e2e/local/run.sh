...
SCRATCH="${FORNAX_E2E_SCRATCH:-$HOME/.cache/fornax-e2e}"
VENV=e2e/local/.venv
BIN=e2e/local/bin
...
cleanup() {
  ...
  [ -n "$DOCKER" ] && "$DOCKER" rm -f fornax-minio >/dev/null 2>&1 || true
}
trap cleanup EXIT
...
"$DOCKER" rm -f fornax-minio >/dev/null 2>&1 || true
...
"$DOCKER" run -d --name fornax-minio -p "$MINIO_PORT:9000" \
  ...
  ghcr.io/latere-ai/minio:RELEASE.2026-09-25T12-30-31Z@sha256:025fb85cf53bad7b734dc05b012c65ce617105248ddd044d8974fb636f2a8824 server /data >/dev/null
...
serve() {
  FORNAX_ENGINE_CMD="$VENV/bin/python -m mlx_lm server --model {model} --host 127.0.0.1 --port {port} --chat-template-args {\"enable_thinking\":false}" \
...
curl -fsS "http://127.0.0.1:$PORT/metrics" | grep -q "fornax_weights_load_seconds" || fail "gauge missing"
