...
func runInstall(rest []string, out, errw io.Writer) error {
	...
	binPath := fs.String("bin", install.DefaultBinPath, "installed fornax binary")
	...
	if err := fs.Parse(rest); err != nil || *path == "" {
		return usagef("usage: fornax install --manifest <manifest.yaml> [--speculator <name|none>] [--print]")
	}
